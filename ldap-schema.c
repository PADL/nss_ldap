...
void _nss_ldap_init_filters()
{
  ...
  snprintf(_nss_ldap_filt_getntohost, LDAP_FILT_MAXSIZ,
	   "(&(objectclass=%s)(%s=%s))", OC (ieee802Device), AT (macAddress), "%s");
  snprintf(_nss_ldap_filt_getetherent, LDAP_FILT_MAXSIZ,
	   "(objectclass=%s)", OC (ieee802Device));
  ...
  snprintf(_nss_ldap_filt_getgrgid, LDAP_FILT_MAXSIZ,
	   "(&(objectclass=%s)(%s=%s))", OC (posixGroup), AT (gidNumber), "%d");
  snprintf(_nss_ldap_filt_getgrent, LDAP_FILT_MAXSIZ,
	   "(&(objectclass=%s))", OC (posixGroup));
#ifdef RFC2307BIS
  ...
  snprintf(_nss_ldap_filt_getgroupsbymember, LDAP_FILT_MAXSIZ,
	   "(&(objectclass=%s)(%s=%s))", OC (posixGroup), AT (memberUid), "%s");
  ...
  snprintf(_nss_ldap_filt_gethostbyaddr, LDAP_FILT_MAXSIZ,
	   "(&(objectclass=%s)(%s=%s))", OC (ipHost), AT (ipHostNumber), "%s");
  snprintf(_nss_ldap_filt_gethostent, LDAP_FILT_MAXSIZ,
	   "(objectclass=%s)", OC (ipHost));
  ...
  snprintf(_nss_ldap_filt_getnetbyaddr, LDAP_FILT_MAXSIZ,
	   "(&(objectclass=%s)(%s=%s))", OC (ipNetwork), AT (ipNetworkNumber), "%s");
  snprintf(_nss_ldap_filt_getnetent, LDAP_FILT_MAXSIZ,
	   "(objectclass=%s)", OC (ipNetwork));
  ...
  snprintf(_nss_ldap_filt_getprotobynumber, LDAP_FILT_MAXSIZ,
	   "(&(objectclass=%s)(%s=%s))", OC (ipProtocol), AT (ipProtocolNumber), "%d");
  snprintf(_nss_ldap_filt_getprotoent, LDAP_FILT_MAXSIZ,
	   "(objectclass=%s)", OC (ipProtocol));

  /* users */
  snprintf(_nss_ldap_filt_getpwnam, LDAP_FILT_MAXSIZ,
	   "(&(objectclass=%s)(%s=%s))", 
	   OC (posixAccount), AT (uid), "%s");
  ...
  snprintf(_nss_ldap_filt_getrpcbynumber, LDAP_FILT_MAXSIZ,
	   "(&(objectclass=%s)(%s=%s))", OC (oncRpc), AT (oncRpcNumber), "%d");
  snprintf(_nss_ldap_filt_getrpcent, LDAP_FILT_MAXSIZ,
	   "(objectclass=%s)", OC (oncRpc));
  ...
  snprintf(_nss_ldap_filt_getservbyport, LDAP_FILT_MAXSIZ,
	   "(&(objectclass=%s)(%s=%s))", OC (ipService), AT (ipServicePort), "%d");
  snprintf(_nss_ldap_filt_getservbyportproto, LDAP_FILT_MAXSIZ,
	   "(&(objectclass=%s)(%s=%s)(%s=%s))",
	   OC (ipService), AT (ipServicePort), "%d", AT (ipServiceProtocol), "%s");
  snprintf(_nss_ldap_filt_getservent, LDAP_FILT_MAXSIZ,
	   "(objectclass=%s)", OC (ipService));
...
void init_hosts_attributes(const char ***hosts_attrs) 
{
...
void init_services_attributes(const char ***services_attrs)
{
...
void init_network_attributes(const char ***network_attrs)
{
...
void init_proto_attributes(const char ***proto_attrs) 
{
...
void init_rpc_attributes(const char ***rpc_attrs)
{
...
void init_ethers_attributes(const char ***ethers_attrs) 
{
...
void init_bp_attributes(const char ***bp_attrs)
{
...
void init_alias_attributes(const char ***alias_attrs) 
{
...
void init_netgrp_attributes(const char ***netgrp_attrs)
{
...
static const char *hosts_attributes[] =
{AT (cn), AT (ipHostNumber), NULL};

static const char *services_attributes[] =
{AT (cn), AT (ipServicePort),
 AT (ipServiceProtocol), NULL};

static const char *network_attributes[] =
...
static const char *ethers_attributes[] =
